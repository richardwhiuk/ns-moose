		...
		for(j = 0; j < size; ++j){
			for(k = 0; k < size; ++k){
				...
				if((k + 1) < size){
					...
					t.bridgeLinks.insert(std::make_pair<long,long>(me, ((i * size * size) + (j * size) ) ));
				}

				if((j + 1) < size){
					t.bridgeLinks.insert(std::make_pair<long,long>(me, ((i * size * size) + ((j + 1) * size) + k) ));
				} else {
					t.bridgeLinks.insert(std::make_pair<long,long>(me, (i * size * size)  + k) );
				}

				if((i + 1) < size){
					t.bridgeLinks.insert(std::make_pair<long,long>(me, (((i + 1) * size * size) + (j * size) + k) ));
				} else {
					t.bridgeLinks.insert(std::make_pair<long,long>(me, ((j * size) + k) ));
				}
			}
		}	
	}
